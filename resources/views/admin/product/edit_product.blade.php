...
@section('header')
    <h3>Edit {{ $name }}</h3>
    <img style="width: 75px; border-radius: 50%;" src="{{ asset('images/products/' . $data->image) }}" alt="">
@endsection
    ...
    <div class="row">
        <form action="{{ route('updateProduct', [$data->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="en_details" value="{{ $data->en_details }}">
                <span class="text-danger">
                    @error('en_details')
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="ar_details" value="{{ $data->ar_details }}">
                <span class="text-danger">
                    @error('ar_details')
                        من فضلك ادخل التفاصيل بالعربيه
                    @enderror
                </span>
            </div>
            <br>
            ...
            <div class="form-group">
                ...
                <select class="form-select form-select-sm mt-3" name="category_id">
                    @foreach ($catgories as $catgory)
                        <option value="{{ $catgory->id }}" @if ($data->category_id == $catgory->id) selected @endif>
                            {{ $catgory->en_name }}
                        </option>
                    @endforeach
                </select>
